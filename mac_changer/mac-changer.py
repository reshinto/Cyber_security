...
def check_change(shell, interface, new_mac, old_mac="hidden"):
    _current = get_MAC(shell, interface)
    if _current == new_mac:
        print(f"[+] MAC address was successfully changed from {old_mac} to {_current}")
    else:
...
def main():
    ...
    check_change(options.shell, options.interface, options.new_mac, old_mac)
